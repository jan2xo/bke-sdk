...
await File.WriteAllLinesAsync(csvPath, new[] { "algorithm,keyMode,concurrent,opsPerSecond,p50Ticks,p95Ticks,p99Ticks,allocatedBytesPerOp,admitted,throttled,memoryDeltaBytes" }
    .Concat(rows.Select(x => $"{x.Algorithm},{x.KeyMode},{x.Concurrent},{x.OpsPerSecond:F2},{x.P50Ticks},{x.P95Ticks},{x.P99Ticks},{x.AllocatedBytesPerOperation:F2},{x.Admitted},{x.Throttled},{x.MemoryDeltaBytes}")));
